...
BEGIN
    ...
    SELECT @somme_prix1 = ((([stri16].[dbo].[PRODUIT].[Prixht_prod] * (1 - [Prom_prod])
                            + [stri16].[dbo].[PRODUIT].[Prixhtprom_prod] * [Prom_prod])*
    ...
    SELECT @somme_prix2 = ((([stri16].[dbo].[PRODUIT].[Prixht_prod] * (1 - [Prom_prod])
                            + [stri16].[dbo].[PRODUIT].[Prixhtprom_prod] * [Prom_prod])*
                            ( 1 + [stri16].[dbo].[PRODUIT].[Tauxtva_prod] / 100))*
                            @qte_com2 )
            FROM [stri16].[dbo].[PRODUIT]
    ...
    -- 3)
    -- Afficher le nbr de fois que les produits d'une commande ont ete commandes

    DECLARE @compteur char(10)
    ;

    -- produit 1
    SELECT @compteur = COUNT([Id_com])
      FROM [stri16].[dbo].[DETAIL_COMMANDE]
      WHERE [Id_prod] = @id_prod1
    ;
    PRINT 'Nombre de commandes de ' + @id_prod1 + ' : ' + @compteur
    ;

    -- produit 2
    SELECT @compteur = COUNT([Id_com])
      FROM [stri16].[dbo].[DETAIL_COMMANDE]
      WHERE [Id_prod] = @id_prod2
    ;
    PRINT 'Nombre de commandes de ' + @id_prod2 + ' : ' + @compteur
    ;
...
-- test
declare @idclient char(10);
declare @idcom int;

execute PRISE_COMMANDE 'nom',
    'prenom',
    'adresse',
    '02-02-1990',
    'id_paiement',
